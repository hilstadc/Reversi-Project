...
bool nullcheck (int cord[2], int row, int col){
  if (cord[0]==0 && row==-1){ //top side of board
    ...
    return false; }
  else if (cord[1]==0 && vol==-1){ //left side of 
    return false; }
  else if (cord[1]==7 && int[1]==1){ //right side of the board
    return false; }
  else{
    return true;
    }
...
int validate (int board[8][8], int cord [2], int race){ //Main function for here
  ...
  bool cool;
  tail[2]; //where 'move' will end there program;
  cool=false;
  f=1;
  e=2;
  ...
  if (cool==false){    // INVALID
    ...
    return 1; //found something and changed it
  }
  
  
// failiure == 0              NULL
// Success == 1              new int[8][8]
